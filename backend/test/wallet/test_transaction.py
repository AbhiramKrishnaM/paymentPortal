from backend.config import MINE_RATE, MINING_REWARD, MINING_REWARD_INPUT
import pytest
...
def test_transaction_update_exceeds_balance():
    ...
    with pytest.raises(Exception, match='Amount exceeds balance'):
        transaction.update(sender_wallet, 'new_transaction', 9001) 
...
def test_valid_reward_transaction():
    test_reward_transaction = Transaction.reward_transaction(Wallet())
    Transaction.is_valid_transaction(test_reward_transaction)
